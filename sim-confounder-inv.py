...
results_path = "Results confounder-sim-inv/Interpretation/"
...
if __name__ == '__main__':
    ...
    # Generate y which is caused by X3
    y = 2 * X3 + np.random.normal(0, 1, n_samples)  # y has a causal relationship with X3

    # Generate a latent variable Z
    Z = y + np.random.normal(0, 1, n_samples)

    # Generate X1 and X2 which are highly correlated with X3 and Z
    X1 = 0.5 * X3 + 0.5 * Z + np.random.normal(0, 0.1, n_samples)
    X2 = 0.5 * X3 + 0.5 * Z + np.random.normal(0, 0.1, n_samples)
